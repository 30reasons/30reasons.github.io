...
<p>
Posters were generously contributed by{% for reason in site.reasons %}{% if forloop.index <= site.currentReason %}{% if forloop.index == site.currentReason and site.currentReason != 1 %} and{% endif %} <a href="{{ reason.url | prepend: site.github.url }}">{% if reason.artistShort %}{{ reason.artistShort }}{% else %}{{ reason.artist }}{% endif %}</a>{% unless forloop.index == site.currentReason or site.currentReason == 2 %},{% endunless %}{% else %}{% break %}{% endif %}{% endfor %}.
</p>
